    ...
    <tr class="bg-white hover:bg-gray-100">
        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
            <input v-if="edit" v-model="nameInput"
                   @blur="edit=false; editComponent($props.id, nameInput, $props.product_id)"
                   @keyup.enter="edit=false; editComponent($props.id, nameInput, $props.product_id)"
                   v-focus
        ...
        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
            <a href="#"
               class="text-indigo-600 hover:text-indigo-900">Edit Sections<span
        ...
        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
            ...
            <button class="text-indigo-600 hover:text-indigo-900"
                    ...
                    type="button"
                    v-on:click="deleteComponent($props.id, index)"
            >Delete
            </button>
        </td>
    </tr>
...
export default {
    ...
    methods: {
        editComponent(id, newValue, product_id) {
            let formData = new FormData();
            formData.append('name', newValue);
            formData.append('product_id', product_id);
            formData.append('_method', 'PUT');
            let action = '/admin/components/' + id;
